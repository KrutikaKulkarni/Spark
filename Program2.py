...
#file2 = file1.map(lambda (x, y) : (filter(remove_punct, x), y) )
...
#file2 = file1.map(lambda (x,y) : (filter(remove_stopwords,x), y) )

#converting to lower case


#file2 = file1.map(lambda (x,y) : (x.lower(), y))


# creating the rdd which splits on :::
file2 = file1.map(lambda bookid : bookid.split(':::'))
file3 = file2.filter(remove_stopwords)

for i in file3.collect():
    print i


#creating rdd by taking only last two fields and spliting them accodingly
#file4 = file3.map(lambda fields : (fields[1].split('::') , fields[2].split(' ')))



#for i in file4.collect():
 #   print i
